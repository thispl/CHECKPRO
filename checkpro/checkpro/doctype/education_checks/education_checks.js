...
frappe.ui.form.on('Education Checks', {
	...
	if (frm.doc.workflow_state=="Report Completed" || frm.doc.workflow_state=="Drop"){
	...
	if(frm.doc.workflow_state=="Draft"||frm.doc.workflow_state=="Entry Completed"||frm.doc.workflow_state=="Entry QC Pending"||frm.doc.workflow_state=="Entry QC Completed"||frm.doc.workflow_state=="Execution Initiated"||frm.doc.workflow_state=="Execution Pending"){
		...
		primary_action: () => {
			...
			console.log("jh")
			frappe.call({
				method:"teampro.custom.app_state",
				args:{
					'name':frm.doc.name,
			...
			d.hide();
			frm.save()
			frappe.confirm(__("Are you sure you want to Mark Not Applicable?"), function () {
				frappe.call({
					method: "teampro.custom.update_case_status",
					args: {
							"case": frm.doc.case_id,
...
if(frm.doc.workflow_state=="Insufficient Data"){
	frm.add_custom_button(__("Clear Insufficiency"), function () {
		frappe.call({
			method: "checkpro.checkpro.doctype.education_checks.education_checks.education_checks_mail",
			args: {
				id: frm.doc.name
			},
		});
...
frm.add_custom_button(__("Drop"), function () {
	...
	primary_action: () => {
		...
		console.log("jh")
		frappe.call({
			method:"teampro.custom.check_status",
			args:{
				'name':frm.doc.name,
...
custom_institute: function(frm) {
    if (frm.doc.custom_institute) {
        frappe.call({
            method: "checkpro.custom.get_vpi_details",
            args: {
                name: frm.doc.name,
                ins: frm.doc.custom_institute
            },
            callback: function(r) {
                if (r.message) {
					console.log(r.message)
                    frm.clear_table("custom_vpi_details");
                    r.message.forEach(function(row) {
                        let child = frm.add_child("custom_vpi_details");
                        child.verified_by = row.verified_by;
                        child.name1 = row.name1;
                        child.designation = row.designation;
                        child.contact = row.contact;
                    });
                    frm.refresh_field("custom_vpi_details");
                }
            }
        });
    }
