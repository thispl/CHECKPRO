...
frappe.ui.form.on('Social Media', {
						...
						frm.add_custom_button(__("Re-Open"), function () {
							let d = new frappe.ui.Dialog({
										...
										fields: [
											{
												...
												fieldname: 'custom_reason_to_reopen',
												fieldtype: 'Small Text',
											},
										],
										primary_action_label: __('Save'),
										primary_action: () => {
											...
											frm.set_value("custom_reason_to_reopen",values.custom_reason_to_reopen)
											frm.set_df_property("custom_reopen_date", "read_only", 1);
											frm.set_df_property("custom_reason_to_reopen", "read_only", 1);
										d.hide();
										frm.save()
										},
									});
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (mandatoryFieldsFilled) {
						console.log("Setting entered_by:", frappe.session.user);
						frm.set_value("entered_by", frappe.session.user);
						console.log("entered_by after setting:", frm.doc.entered_by);
						...
						function(){
							console.log("Resetting fields after cancellation");
	...
	if(frm.doc.workflow_state=="Draft"||frm.doc.workflow_state=="Entry Completed"||frm.doc.workflow_state=="Entry QC Pending"||frm.doc.workflow_state=="Entry QC Completed"||frm.doc.workflow_state=="Execution Initiated"||frm.doc.workflow_state=="Execution Pending"){
		...
		primary_action: () => {
			let values = d.get_values();
			frappe.call({
				method:"teampro.custom.na_status",
				args:{
					'name':frm.doc.name,
	...
	primary_action(values) {
		...
		frm.set_value('insufficiency_reported_on', frappe.datetime.now_datetime());
		frm.save();
	...
	function(){
		...
		frm.set_value('insufficiency_reported_on', '');
		frm.save();
	}
)
...
if(frm.doc.workflow_state=="Insufficient Data"){
	frm.add_custom_button(__("Clear Insufficiency"), function () {
		frappe.call({
			method: "checkpro.checkpro.doctype.social_media.social_media.social_media_mail",
			args: {
				id: frm.doc.name
			},
		});
        ...
        var insuff_date = frappe.datetime.nowdate()
		frm.set_value('insuff_cleared_on', insuff_date);
		frm.set_value('insuff_closed', insuff_date);
		frm.set_value('custom_allocation_date', frappe.datetime.nowdate());
		frappe.call({
...
frm.add_custom_button(__("Drop"), function () {
	...
	primary_action: () => {
		let values = d.get_values();
		frappe.call({
			method:"teampro.custom.drop_status",
			args:{
				'name':frm.doc.name,
