...
frappe.ui.form.on('Employment', {
	epi_date_of_relieving(frm){
	    var date_of_joining = frm.doc.epi_date_of_joining;
	    var date_of_relieving = frm.doc.epi_date_of_relieving
		...
		if(date){
        frm.set_value("epi_period_of_employment", year +" "+ "Years" +" " +month +" "+"Months"+" "+ date+" " +"Days" )
		}
		else{
			frm.set_value("epi_period_of_employment","")
		}
	},
	refresh(frm) {
	...
	if(frm.doc.workflow_state=="Draft"||frm.doc.workflow_state=="Entry Completed"||frm.doc.workflow_state=="Entry QC Pending"||frm.doc.workflow_state=="Entry QC Completed"||frm.doc.workflow_state=="Execution Initiated"||frm.doc.workflow_state=="Execution Pending"){
		...
		primary_action: () => {
			...
			console.log("jh")
			frappe.call({
				method:"teampro.custom.state_report",
				args:{
					'name':frm.doc.name,
...
if(frm.doc.workflow_state=="Insufficient Data"){
	frm.add_custom_button(__("Clear Insufficiency"), function () {
		frappe.call({
			method: "checkpro.checkpro.doctype.employment.employment.employment_check_mail",
			args: {
				id: frm.doc.name
			},
		});
...
frm.add_custom_button(__("Drop"), function () {
	...
	primary_action: () => {
		...
		console.log("jh")
		frappe.call({
			method:"teampro.custom.document_state",
			args:{
				'name':frm.doc.name,
