...
class Case(Document):
                ...
                if not checks:
                    for cp_check in cp_checks:
                        for unit in range(int(cp_check.units)):
                            ...
                            self.annexture_required=cp_check.annexure
                            create_verify_check = frappe.new_doc(cp_check.check_name)
                            if cp_check.check_name=='Identity Aadhar':
                                create_verify_check.epi_aadhar_name=self.case_name
                                create_verify_check.epi_aadhar_dob=self.date_of_birth
                                create_verify_check.epi_aadhar_address=self.address
                            if cp_check.check_name=='Address Check':
                            ...
                            create_verify_check.epi_address=self.address
                    ...
                    for ch in checks:
                        ...
                        if c.check_name=='Address Check':
                            if checks.address is None :
                                checks.update({
                        ...
                        if c.check_name=='Criminal':
                            checks.update({
                                'epi_address':self.address,
                                })
                        if checks.allocated_to is None:
                            checks.update({
                                ...
                                "custom_allocation_date":self.custom_allocation_date,
    ...
    # def autoname(self):
    #     get_batch=frappe.db.get_all("Batch",{'name':self.name},['customer','name'])
    #     get_customer=frappe.db.get_value("Customer",{'name':get_batch},['short_code'])
    #     get_initiation_date=frappe.db.get_value("Batch",{'name':self.name },['expected_start_date'])
    #     new=frappe.db.get_value("Batch",{'name':self.name},['no_of_cases'])
    #     new_sequence=1
    #     formatted_sequence = f"{new_sequence:0{new}d}"
    #     self.name = self.get_customer + " - " + self.get_initiation_date + "-" +self.formatted_sequence
...
def check_status(name,check_package):
    ...
    for c in check_package.checks_list:
        ...
        checks = frappe.get_all(c.check_name,{"case_id":name},['entry_status','report_status','name'])
...
def update_per_comp(name):
    ...
    total_cases = 0
    for i in list:
        doc=frappe.get_all(i,{"case_id":name},["name","workflow_state"])
        for j in doc:
            total_cases+=1
