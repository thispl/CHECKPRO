...
def update_task_count(doc, method):
    if doc.task:
        ...
        psl = frappe.db.count('Candidate', {'task': doc.task, 'pending_for':'Proposed PSL'}) or 0
...
def send_mail_with_attachment_project(filename, file_content):
    ...
    attachments = [{"fname": filename + '.xlsx', "fcontent": file_content}]
    frappe.sendmail(
        recipients=["dineshbabu.k@groupteampro.com","sangeetha.a@groupteampro.com","sangeetha.s@groupteampro.com","annie.m@groupteampro.com","vijiyalakshmi.k@groupteampro.com",'keerthana.k@groupteampro.com'],
...
#     candidate=frappe.get_all("Task",{"task":name},["name"])
#     for i in candidate:
#         if check=='1':
...
def closure_mail(subject,id,action_taken,live,et,at,revision,service,proof,allocated=None,project=None,issue=None,domain=None,spoc=None,reason=None):
    if service=='IT-SW':
        percentage=et_at_calculation(id, et, at, allocated)
...
@frappe.whitelist()
def nc_for_reopen(cause=None,id=None,allocated=None):
    if allocated:
        emp_id=frappe.db.get_value("Employee",{'user_id':allocated},['name'])
        reopen_cause='Task : %s Re-Open ' % (id)
...
@frappe.whitelist()
def et_at_calculation(id, et, at, allocated):
...
#     args = frappe.local.form_dict
#     project_spoc=frappe.db.get_value("Project",{"name":args.project},['spoc'])
#     candidate_mail=frappe.db.get_value("Candidate",{"name":args.candidate_id},['mail_id'])
#     table = '<table text-align="center" border="1" width="100%" style="border-collapse: collapse;text-align: center;">'
#     table += '<tr style="background-color: #87CEFA"><td style="width: 45%; font-weight: bold; text-align: center;">Closure ID</td><td style="width: 30%; font-weight: bold; text-align: center;">Candidate Name</td><td style="width: 25%; font-weight: bold; text-align: center;">Project</td><td style="width: 25%; font-weight: bold; text-align: center;">Status</td></tr>'
#     table += """<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>""" % (args.closure_id,args.candidate_name,args.project,args.status)        
#     table += '</table>'
...
#     frappe.sendmail(
#         # recipients=[project_spoc,"sangeetha.a@groupteampro.com"],
#         subject=subject,
#         message=message,
#         # attachments=attachments,
#         attachments=[{
...
#     )
#     header_row = ["Closure ID", "Candidate Name", "Project", "Status"]
...
#     data_row = [args.closure_id or '', args.candidate_name or '', args.project or '', args.status or '']
#     ws.append(data_row)
#     xlsx_file = BytesIO()
#     wb.save(xlsx_file)
#     xlsx_file.seek(0)
...
@frappe.whitelist()
def get_allocated_tasks(date,name,service):
    total=0
    task_id=frappe.db.get_all("Task",{"custom_production_date":date,"service":service},['*'],order_by='cb asc')
    task_det=frappe.db.get_all("Task",{"custom_production_date":date,"service":service},['*'],order_by='cb asc',group_by='cb asc')
    parent_doc = frappe.get_doc("Daily Monitor", name)
    parent_doc.task_details=[]
    # data = '<table border="1" width="50%" style="border-collapse: collapse;text-align:center;">'
    # data+='''<tr style="background-color: #0f1568 ;color: white;text-align:center;font-size: 12px;">
    #         <td style='width:1% ;text-align:center;'><b>CB</b></td>
    #         <td style='width:1% ;text-align:center;'><b>APH</b></td>
    #         <td style='width:1% ;text-align:center;'><b>RT</b></td>
    #     </tr>
    #     '''
    parent_doc.dm_summary=[]
    for i in task_id:
        task_status=frappe.db.get_value("Task",{"name":name},["status"])
        if ["Open","Working","Overdue"] in task_status:
            parent_doc.append("task_details", {"a_task_type": "OPS","id": i.name})
        elif ["Pending Review","Client Review"] in task_status:
            parent_doc.append("task_details", {"a_task_type": "CS","id": i.name})
        else:
            parent_doc.append("task_details", {"id": i.name})
    ...
    for k in task_det:
        ...
        sum_et=frappe.db.sql("""select sum(rt) as et from `tabTask` where cb=%s and custom_production_date=%s group by cb""",(k.cb,date), as_dict=True)
        total+=sum_et[0].et
        if actual_aph is not None:
            percent=sum_et[0].et/actual_aph
            parent_doc.append("dm_summary",{'d_cb':k.cb,'d_aph':actual_aph,'d_rt':sum_et[0].et,'d_actual_time_taken':'','rt_vs_aph_':round(percent,2)})
        else:
            parent_doc.append("dm_summary",{'d_cb':k.cb,'d_aph':actual_aph,'d_rt':sum_et[0].et,'d_actual_time_taken':'','rt_vs_aph_':'0'})
           
    #     if parent_doc.aph is not None :
    #         value=parent_doc.aph
    #         data+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td>' %(k.cb,value or '8',sum_et[0].et)
    #     else:
    #         data+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td>' %(k.cb,actual_aph or '',sum_et[0].et)
    # data+='<tr style="font-size: 14px;" ><td colspan=2>Total</td><td>%s</td>'%(total)
    # data += "</table>"
...
@frappe.whitelist()
def update_dsr(date, name,task_type,service):
    parent_doc = frappe.get_doc("Daily Monitor", name)
    parent_doc.task_details = []
    task_id_list = frappe.db.get_all("Task", {"custom_production_date": date}, ['*'], order_by='cb asc')
    task_list = frappe.db.get_all("Task", {"custom_production_date": date}, ['*'], order_by='cb asc',group_by='cb asc')
    ...
    for issue in issues:
        parent_doc.append("task_details", issue)
    ...
    for j in task_list:
            employee_id=frappe.db.get_value('Employee',{'short_code':j.cb},['name'])
            timesheet = frappe.db.get_value("Timesheet", {'start_date': date, 'employee':employee_id}, ['total_hours'])      
            actual_aph=frappe.db.get_value('Employee',{'short_code':j.cb},['custom_aph'])
            sum_et=frappe.db.sql("""select sum(rt) as et from `tabTask` where cb=%s and custom_production_date=%s group by cb""",(j.cb,date), as_dict=True)
            if timesheet and actual_aph is not None:
                percent=float(timesheet)/float(actual_aph)
                parent_doc.append("dm_summary",{'d_cb':j.cb,'d_aph':actual_aph,'d_rt':sum_et[0].et,'d_actual_time_taken':timesheet,'rt_vs_aph_':round(percent,2)})
            else:
                parent_doc.append("dm_summary",{'d_cb':j.cb,'d_aph':actual_aph,'d_rt':sum_et[0].et,'d_actual_time_taken':timesheet,'rt_vs_aph_':'0'})

            # total+=sum_et[0].et
    #         if parent_doc.aph is not None :
    #             value=parent_doc.aph
    #             table+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td><td>%s</td>' %(j.cb,value or '8',sum_et[0].et,round(timesheet,2) if timesheet is not None else '-')
    #         else:
    #             table+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td><td>%s</td>' %(j.cb,actual_aph or '',sum_et[0].et,round(timesheet,2) if timesheet is not None else '-')
    # table+='<tr style="font-size: 14px;" ><td colspan=2>Total</td><td>%s</td>'%(total)
    # table += "</table>"
    parent_doc.save()
    frappe.db.commit()
    # return table
...
def dpr_task_mail(date,name):
    ...
    task=frappe.db.get_all("Task",{"custom_production_date":date},['*'],group_by='cb asc',order_by='cb asc')
    ...
    if task_data.dsr_check==1:
        ...
        frappe.db.set_value('Daily Monitor',name,'workflow_state','Submitted')
        ...
        for i in task_data.task_details:
            ...
            count+=1
        data += '</table>'
        task_det=frappe.db.get_all("Task",{"custom_production_date":date},['*'],order_by='cb asc',group_by='cb asc')
        for k in task_det:
            actual_aph=frappe.db.get_value('Employee',{'short_code':k.cb},['custom_aph'])
            sum_et=frappe.db.sql("""select sum(rt) as et from `tabTask` where cb=%s and custom_production_date=%s group by cb""",(k.cb,date), as_dict=True)
            total+=sum_et[0].et
            if task_data.aph is not None :
                value=task_data.aph
                table+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td>' %(k.cb,value or '8',sum_et[0].et)
            else:
                table+='<tr style="font-size: 14px;"><td>%s</td><td>%s</td><td>%s</td>' %(k.cb,actual_aph or '',sum_et[0].et)
        table+='<tr style="font-size: 14px;" ><td colspan=2>Total</td><td>%s</td>'%(total)
        table+='</table>'
        frappe.sendmail(
                # sender='abdulla.pi@groupteampro.com',
...
Please find the below DPR for {} for your kind reference and action, ensure all the Tasks allocated on time and as per the requirement, for each Revision and AT going beyond 150% there will be NC applied and accumulated NC will be reviewed every week and directly affects your Performance.<br><br>
        ...
        frappe.db.set_value('Daily Monitor',name,'workflow_state','DSR Pending')
